...
> **Login online exige OAuth.** GitHub Pages não hospeda o servidor OAuth sozinho. Opções:
> - Hospedar [decap-cms-oauth-provider-node](https://github.com/decaporg/decap-cms-oauth-provider-node) (Render, Railway, etc.) e adicionar `backend.base_url` no `admin/config.yml`
> - Ou editar só localmente com `npm run cms` (mais simples para equipe pequena)

Documentação: [Decap CMS + GitHub](https://decapcms.org/docs/github-backend/)
...
- [ ] OAuth para admin online (configurar quando quiser editar direto no GitHub Pages)
